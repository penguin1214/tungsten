...
namespace Tungsten {
	...
	void PhotonMapIntegrator::tracePhotons(uint32 taskId, uint32 numSubTasks, uint32 threadId, uint32 sampleBase) {
		...
		for (uint32 i = 0; i < photonsToCast; ++i) {
			...
			/// relationship between tread-local photonToCast and data.*Range?
	...
	void PhotonMapIntegrator::renderSegment(std::function<void()> completionCallback) {
		...
		if (!_surfaceTree) {
			/// If kdtree is not built yet, trace photon
	...
	void PhotonMapIntegrator::writePhotonToFile() {
		// store volumeRange._dst
		// use volume point
		/*std::ofstream file("volume_photon.txt");
		if (file.is_open()) {
			for (auto &p : _volumePhotons) {
				file << p.pos << std::endl;
			}
		}
		file.close();*/
		...
		std::ofstream file;
		file.open("surface_photon.txt");
		if (file.is_open()) {
			for (auto &p : _surfacePhotons) {
				file << p.pos.x() << "," << p.pos.y() << "," << p.pos.z() << std::endl;
			}
		}
		file.close();

		/*file.open("path_photon.txt");
		if (file.is_open()) {
			for (auto &p : _pathPhotons) {
				file << p.pos.x() << "," << p.pos.y() << "," << p.pos.z() << std::endl;
			}
		}
		file.close();*/
	std::cout << "photon map file saved." << std::endl;
	}
}
